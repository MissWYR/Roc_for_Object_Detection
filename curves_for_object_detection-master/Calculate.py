...
def match(resultsfile, groundtruthfile, show_images):
    ...
    # test_set=set()
    # for test1 in range(len(results)):
    #     # print(results[test1][0])
    #     test_set.add(results[test1][0])
    #
    # for test2 in range(len(groundtruth)):
    #     if groundtruth[test2][0] not in test_set:
    #         print(groundtruth[test2][0]+'false')
    ...
    for i in range(len(results)):
        ...
        for j in range(2, len(results[i])): # 对于一张图片中的每一个检测框
            ...
            for l in range(len(results)):
                if results[i][0]==groundtruth[l][0]:  #匹配照片名字是否一样


                    for k in range(2, len(groundtruth[l])): # 去匹配这张图片中的每一个标注框
                        groundtruthbox = groundtruth[l][k]
...
def plot(tf_confidence, num_groundtruthbox):
    ...
    # print(fp_list)
    plt.plot(fp_list, recall_list, color='darkorange', label='3') ###kf_r_1836_rs(th0.5_file)
    ####
